...
from app.services.order_service import TaskService
...
async def consume(loop: AbstractEventLoop) -> AbstractRobustConnection:
    ...
    task_created_queue = await channel.declare_queue('shendyapin_order_created_queue', durable=True)

    await task_created_queue.consume(process_created_task)
    print('Начало работы RabbitMq')
